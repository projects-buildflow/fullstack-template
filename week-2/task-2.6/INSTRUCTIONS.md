...
export function useKeyboardNavigation({
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    // TODO: Handle ArrowUp - decrement focusedTaskIndex
    // TODO: Handle ArrowDown - increment focusedTaskIndex
    // TODO: Handle ArrowLeft - decrement focusedColumnIndex, reset task index
    // TODO: Handle ArrowRight - increment focusedColumnIndex, reset task index
    // TODO: Handle Enter - call onEditTask
    // TODO: Handle Delete/Backspace - call onDeleteTask (check not in input)
    // TODO: Handle Space - call onSelectTask
  }, [columns, onSelectTask, onEditTask, onDeleteTask]);
  ...
  const currentTaskId = columns[focusedColumnIndex]?.taskIds[focusedTaskIndex];
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-sm border p-4 cursor-pointer transition-all ${
        isFocused ? 'ring-2 ring-blue-500 border-blue-500' : 'border-gray-200 hover:shadow-md'
      }`}
      onClick={onClick}
...
export function Board() {
  ...
  const { focusedColumnIndex, focusedTaskIndex, focusedTaskId } = useKeyboardNavigation({
    columns: state.columns,
    onSelectTask: (taskId) => console.log('Selected:', taskId),
    onEditTask: (taskId) => setEditingTask(state.tasks[taskId]),
    onDeleteTask: (taskId) => {/* Open delete confirmation */},
  });

  return (
    <div role="application" aria-label="Kanban board" className="flex gap-6 overflow-x-auto p-6">
      {/* TODO: Map columns with aria-label */}
      {/* TODO: Pass isFocused prop to TaskCard based on indices */}
    </div>
  );
}
```
...
- Prevent default on arrow keys to avoid page scroll
